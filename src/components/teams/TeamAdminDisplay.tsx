...
import { deleteTeamAdmin, flipLockAdmin, removeTeamMemberAdmin } from "../../utils/apis/firebase"
import Modal from "../Modal"
import { LockClosedIcon, TrashIcon } from "@heroicons/react/24/solid"
import toast from "react-hot-toast"


export const TeamAdminDisplay = (props: { teamPage: TeamFormationProps, teamPages: TeamFormationProps[], setTeamPages: Dispatch<TeamFormationProps[]>}) => {
    const {
        auth: { user },
      } = useAuth()
      ...
      <Modal
        ...
        actionFunc={() => {
                flipLockAdmin(
                    ...
                    const newTeamPages = props.teamPages.filter((tPage) => tPage.teamName !== newTeamPage.teamName)
                    props.setTeamPages([newTeamPage, ...newTeamPages])
                    toast.success(newTeamPage.lockedIn ? "Team Locked In" : "Team Lock Removed")
                }).catch((error) => {
      ...
      <Modal
        ...
        actionFunc={() => {
            deleteTeamAdmin(
                ...
                const newTeamPages = props.teamPages.filter((tPage) => tPage.teamName !== props.teamPage.teamName)
